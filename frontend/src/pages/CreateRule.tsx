...
export default function CreateRule() {
  ...
  const [form, setForm] = useState<FormData>({
    ...
    sourceConnectionId: '',
    sourceProvider: '',
    sourcePath: '/Camera Uploads',
    fileTypes: ['photos'],
    filePattern: '',
    targetConnectionId: '',
    targetProvider: '',
    targetPath: '/Organised',
    schedule: '0 * * * *',
    deleteSource: false,
    recursive: false,
  });
  ...
  const canNext = (): boolean => {
    ...
    if (step === 3) return true; // pattern is optional
  ...
  return (
    <div className="max-w-xl mx-auto space-y-6">
      {/* Header */}
      <div>
        <h1 className="text-2xl font-bold text-slate-900">Create Processing Rule</h1>
        <p className="text-slate-500 text-sm mt-0.5">
          Define a source, processing type, and destination.
        </p>
      </div>
        ...
        {step === 2 && (
          <div>
            ...
            <p className="text-xs text-slate-500 mt-4">
              Files will be renamed to <code className="bg-slate-100 px-1 rounded">YYYY-MM-DD_HH-mm-ss_originalname.ext</code> using the embedded timestamp.
            </p>
          </div>
        )}
        ...
        {step < STEPS.length - 1 ? (
          ...
          >
            {saving ? (
              ...
              <><Layers className="w-4 h-4" />Create Rule</>
            )}
          </button>
        )}
      </div>
